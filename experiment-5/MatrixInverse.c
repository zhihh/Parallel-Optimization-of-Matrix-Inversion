...
#define MAX_Order 1000 /*最大阶数*/
int num_t; /* num of threads */
typedef struct {
	double SquareMatrix [MAX_Order][MAX_Order];/*方阵定义*/
	int order; /*矩阵阶数*/
}matrix;
...
//矩阵输入
void InputMode_0(matrix* m);	//矩阵手动输入
void InputMode_1(matrix* m);	//矩阵自动生成

//矩阵求逆计算通用过程
double Loop_Compute(matrix* m,double entity, int lower, int upper, int row, int column);	//串行
double Loop_Compute_omp(matrix* m,double entity, int lower, int upper, int row, int column);	//并行优化过程，openmp
...
//串行测试
void test_serial(matrix* m);
//并行测试
void test_parallel(matrix* m);
...
double Loop_Compute(matrix* m,double entity, int lower, int upper, int row, int column){
	...
	*/
	/* 
		此循环计算过程需要重复order的平方次，过程内的循环仅仅是order次
		【注】此过程外的order的平方次因有依赖，无法并行；

		对于omp而言，并行优化范围仅仅在循环内，仅当order到达一定程度，循环内才是正优化，否则是负优化，再加上order的平方次的累积，负优化效果显著，当然正优化效果也是显著的；
		对于mpi而言，order的平方次循环可以共用一次并行初始化，循环内正优化要求的order更低
...
//循环计算过程openmp优化
double Loop_Compute_omp(matrix* m,double entity, int lower, int upper, int row, int column){
	/* entity是过程变量
		lower 循环下限
		upper 循环上限
	*/

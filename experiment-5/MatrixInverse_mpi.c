...
double Trap(matrix* m, int n, double lower, int row, int column);   //mpi并行循环
...
int main(){
	...
	if(my_rank == 0){
		...
		for(int i = 1; i < comm_sz; i++){
			MPI_Send(m->SquareMatrix, m->order*m->order, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
		}
		...
		//t2 = MPI_Wtime();
		Print_Matrix(m);
		//printf("Finished in %lfseconds\n", t2-t1);
	}else{
		MPI_Recv(m->SquareMatrix, m->order*m->order, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		Print_Matrix(m);
...
double Loop_Compute(matrix* m,double entity, int lower, int upper, int row, int column){
	...
	*/
    ...
    local_n = (upper - lower)/comm_sz;
 
    local_int = Trap(m,local_n, lower, row, column);

	if (my_rank != 0){
        MPI_Send(&local_int, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
        MPI_Recv(&entity, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }else{
		printf("rank %d, local_int: %lf\n", my_rank, local_int);
        ...
        entity = total_int;
		//printf("rank %d, entity: %lf\n", my_rank, entity);

        for (int i = 1; i < comm_sz; i++)
        {
            MPI_Send(&entity, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
        }
...
double Trap(matrix* m, int n, double lower, int row, int column){
    double temp = 0;
	printf("rank:%d\n",n);

    for(int i = lower+my_rank*n; i < lower+(my_rank+1)*n; i++){
		temp += m->SquareMatrix[row][i] * m->SquareMatrix[i][column];

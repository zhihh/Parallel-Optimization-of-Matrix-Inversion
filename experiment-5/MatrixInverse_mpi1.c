	...
	*/
    ...
    if(my_rank == 0){
        ...
        for(int i = 0; i < m->order; i++){
            for(int j = 0; j < m->order; j++){
                if(i <= j){
                    MPI_Recv(&m->SquareMatrix[i][j], 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                }
            }
	    }
    }else{
        ...
        for(int i = 0; i < m->order; i++){
            for(int j = 0; j < m->order; j++){
                if(i <= j){
                    MPI_Send(&m->SquareMatrix[i][j], 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
                }
            }
	    }

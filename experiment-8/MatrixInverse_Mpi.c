//MPI编程
/*
    编译方法:   mpicc -g -Wall -o MatrixInverse_Mpi MatrixInverse_Mpi.c
    运行方法:   mpiexec -n 2 MatrixInverse_Mpi 输入文件 输出文件  
                例如：mpiexec -n 2 MatrixInverse_Mpi in.txt out.txt  
...
//	printf("MatrixInverse of Serial finished in : %lf seconds\n", t1 - t0);
...
//	Print_Matrix(m);
        ...
        for(int i = 0; i < m->order; i++){
            for(int j = 0; j < m->order; j++){
                if(i >= j){
					...
					m->SquareMatrix[m->order-1 - i][m->order-1 - j] = 0;
				}
            }
	    }
    }
	double temp[m->order];
	for(int i = 0; i < m->order; i++){
		MPI_Allreduce(m->SquareMatrix[i], temp, m->order, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		for(int j = 0; j < m->order; j++){
			m->SquareMatrix[i][j] = temp[j];
		}
	}
//	Print_Matrix(m);

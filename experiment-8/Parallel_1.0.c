//omp与mpi混合编程
/*
    编译方法:   mpicc -g -Wall -o Parallel_1.0 Parallel_1.0.c -fopenmp
    运行方法:   mpiexec -n 进程数(推荐2) Parallel_1.0 线程数 读入文件 输出文件
                例如：mpiexec -n 2 Parallel_1.0 8 in.txt out.txt
...
FILE *fp_in, *fp_out;//fp_in为矩阵输入文件、fp_out为矩阵输出文件；
...
void Mode_2(matrix* m);//文件读写矩阵

double Loop_Compute(matrix* m,double entity, int lower, int upper, int row, int column);//循环计算过程
double Loop_Compute_Serial(matrix* m,double entity, int lower, int upper, int row, int column);//线性循环计算过程
void IO_Matrix(matrix* m);//矩阵的输入输出
int read_order(FILE* fp);//读取文件，获取矩阵结束

int my_rank, comm_sz;

//第一次参数：矩阵的阶  第二个参数：线程数
int main(int argc, char* argv[]){
  
	matrix o;
	matrix* m = &o;
	num_t = strtol(argv[1], NULL, 10);
	fp_in = fopen(argv[2], "r");
	fp_out = fopen(argv[3], "w");

	m->order = read_order(fp_in);
	...
	//由于内存空间不连续，只能如此广播
	for(int i = 0; i < m->order; i++){
		MPI_Bcast(m->SquareMatrix[i],m->order,MPI_DOUBLE,0,MPI_COMM_WORLD);
	...
	for(int i = 0; i < m->order; i++){
		for(int j = 0; j < m->order; j++){
			fprintf(fp_out, "%lf", m->SquareMatrix[i][j]);
			if(j < m->order-1){
				fprintf(fp_out,",");
			}
		}
		...
		printf("MatrixInverse of Parallel finished in : %lf seconds\n", t1 - t0);
	}
...
double FindElement_Inverse_O(matrix* m, int row, int column){
	...
	entity_Inverse_O = Loop_Compute_Serial(m, entity_Inverse_O, i, m->order, row, column);
...
double Loop_Compute(matrix* m,double entity, int lower, int upper, int row, int column){
	/* entity是过程变量
		...
		upper 循环上限
	*/
	//要保证所有进程都能到这里，所以如果这里用mpi，就只能这样了
    int delta = (upper - lower)/comm_sz;
    
    double sum = 0, temp = 0;
    //防止当传入的entity非零时，非零数重复相加

    for(int i = lower + my_rank*delta; i < (my_rank==(comm_sz-1)?upper:(lower+(my_rank+1)*delta)); i++){
        sum += m->SquareMatrix[row][i] * m->SquareMatrix[i][column];

    }
    MPI_Allreduce(&sum, &temp, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
//循环计算过程
double Loop_Compute_Serial(matrix* m,double entity, int lower, int upper, int row, int column){
	/* entity是过程变量
		lower 循环下限
		upper 循环上限
	*/
